...
<?php
    if(!empty($wiersz1)){
    echo "ostatnie błędne logowanie: ",$wiersz1['datagodzina']," <hr>";
...
for($x = 2; $x < $arrlength; $x++) {
  ...
  if (is_file("/zad7/users/$usr/$files[$x]")){
    echo "<a href='/zad7/users/$usr/$files[$x]' download='$files[$x]'>$files[$x]</a><br>";
  }else{ 
      echo $files[$x],"<br>";
      $dir2= "/zad7/users/$usr/$files[$x]";
      $files2 = scandir($dir2);
      $arrlength2 = count($files2);
        for($y = 2; $y < $arrlength2; $y++) {
        
        if (is_file("/zad7/users/$usr/$files[$x]/$files2[$y]")){
        echo "--><a href='/zad7/users/$usr/$files[$x]/$files2[$y]' download='$files2[$y]'>$files2[$y]</a>";
        }else{ 
            echo "-->",$files2[$y];
        }
            echo "<br>";
...
<b>Wybierz miejsce docelowe zapisu:</b><br>
<?php
if (is_dir($dir)) {
    if ($dh = opendir($dir)) {
        while (($file = readdir($dh)) !== false) {
            if(is_dir("/zad7/users/$usr/$file") && $file != '.' && $file != '..'){
